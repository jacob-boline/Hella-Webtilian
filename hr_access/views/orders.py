...
def account_get_order_receipt(request, order_id: int):
    ...
    return render(
        request, "hr_shop/checkout/_order_receipt_modal.html", {"order": order, "items": items, "customer": order.customer, "address": order.shipping_address, "is_guest": is_guest}
    )
...
def account_get_unclaimed_orders(request):
    ...
    if not email:
        return HttpResponse(status=400, headers={"HX-Trigger": json.dumps({"showMessage": "No email address is associated with your account."})})
...
def account_submit_claim_unclaimed_orders(request):
    ...
    if not target_ids:
        return HttpResponse(status=400, headers={"HX-Trigger": json.dumps({"showMessage": "You must select one or more orders in order to claim."})})
