...
export function initAboutCarousel(root = document) {
    ...
    function highlightThumb(i) {
        ...
        // Only show prev / current / next <li> elements
        if (thumbLis.length === thumbs.length) {
            thumbLis.forEach((li, idx) => {
                const visible =
                    idx === i || idx === prevIndex || idx === nextIndex;
                li.classList.toggle('is-visible', visible);
            });
        }

        // If you still want centering behavior when there *are* more thumbs,
        // you can keep this fallback (optional):
        if (!thumbLis.length) {
    ...
    // ---------- nav helpers ----------
    const scrollOne = (dir) => {
        // With only 3 visible thumbs, scrolling is optional / no-op.
        if (!thumbsUl) return;
    ...
    // Keep active thumb centered / visible on resize
    const onResize = () => {
        if (!thumbs.length) return;
        highlightThumb(index);
    };
...
function tryInitCarousel(root = document) {
    try {
        ...
        if (window.hrSite && typeof window.hrSite.reflowParallax === 'function') {
            ...
            // Optional: remove this alert once you're happy
