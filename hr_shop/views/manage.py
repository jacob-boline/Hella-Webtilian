...
from hr_shop.forms import ProductEditForm, ProductOptionTypeForm, ProductOptionValueForm, ProductQuickForm, ProductVariantForm
...
def _render_product_list(request, form=None):
    ...
    return render(request, "hr_shop/manage/_pm_product_list.html", {"products": products, "form": form or ProductQuickForm()})


def _render_product_panel(request, product, *, product_form=None, variant_form=None, option_type_form=None, variant_form_overrides=None, reset_option_type_panel=False):
    option_types = product.option_types.all().order_by("position", "id")
    variants = product.variants.all().order_by("id").prefetch_related("variant_options__option_value__option_type")
    ...
    for variant in variants:
        ...
        variant_forms.append((variant, form_override or ProductVariantForm(instance=variant)))
...
def _render_option_type_panel(request, option_type, *, option_type_form=None, value_form=None, value_forms=None):
    values = option_type.values.all().order_by("position", "id")

    if value_forms is None:
        value_forms = [(val, ProductOptionValueForm(instance=val)) for val in values]

    return render(request, "hr_shop/manage/_pm_option_type_panel_partial.html", {
...
def create_product(request):
    ...
    if form.is_valid():
        product = form.save()
        log_event(logger, logging.INFO, "manage.product.created", product_id=product.id)
...
def update_product(request, pk):
    ...
    if form.is_valid():
        form.save()
        log_event(logger, logging.INFO, "manage.product.updated", product_id=product.id)
...
def create_option_type(request, product_pk):
    ...
    if form.is_valid():
        ...
        opt_type.save()
        log_event(logger, logging.INFO, "manage.option_type.created", product_id=product.id, option_type_id=opt_type.id)
...
def update_option_type(request, pk):
    ...
    if form.is_valid():
        form.save()
        log_event(logger, logging.INFO, "manage.option_type.updated", option_type_id=opt_type.id, product_id=opt_type.product_id)
        return _render_option_type_panel(request, opt_type)
...
def delete_option_type(request, pk):
    ...
    opt_type.delete()
    log_event(logger, logging.INFO, "manage.option_type.deleted", option_type_id=option_type_id, product_id=product_pk)
...
def create_option_value(request, option_type_pk):
    ...
    if form.is_valid():
        ...
        value.save()
        log_event(logger, logging.INFO, "manage.option_value.created", option_type_id=opt_type.id, option_value_id=value.id)
...
def update_option_value(request, pk):
    ...
    if form.is_valid():
        form.save()
        log_event(logger, logging.INFO, "manage.option_value.updated", option_value_id=val.id, option_type_id=val.option_type_id)
    ...
    value_forms = [(val, form)] + [(v, ProductOptionValueForm(instance=v)) for v in option_type.values.exclude(pk=val.pk).order_by("position", "id")]
    return _render_option_type_panel(request, option_type, value_forms=value_forms)
...
def delete_option_value(request, pk):
    ...
    val.delete()
    log_event(logger, logging.INFO, "manage.option_value.deleted", option_value_id=option_value_id, option_type_id=opt_type_pk)
...
def create_variant(request, product_pk):
    ...
    if form.is_valid():
        ...
        variant.save()
        log_event(logger, logging.INFO, "manage.variant.created", product_id=product.id, variant_id=variant.id)
...
def update_variant(request, pk):
    ...
    if form.is_valid():
        form.save()
        log_event(logger, logging.INFO, "manage.variant.updated", product_id=product.id, variant_id=variant.id)
        return _render_product_panel(request, product)

    return _render_product_panel(request, product, variant_form_overrides={variant.pk: form})
...
def delete_variant(request, pk):
    ...
    variant.delete()
    log_event(logger, logging.INFO, "manage.variant.deleted", product_id=product_pk, variant_id=variant_id)
