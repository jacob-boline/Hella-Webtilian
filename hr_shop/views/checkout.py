...
def _get_or_create_customer(email: str, user) -> Customer:
    """
    ...
    if user and user.is_authenticated and customer.user is None:
        customer.user = user
...
def checkout_create_order(request):
    ...
    customer = _get_or_create_customer(email, user)
